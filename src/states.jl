...
function PrimitiveState(; ρ::Real, u::Real, p::Real)
    PrimitiveState(ρ, u, p)
end # kwargs
function PrimitiveState(W::AbstractVector{<:Real})
    length(W) == 3 ||
        throw(ArgumentError("state vector W must only contain 3 elements (ρ, u, p)"))
    return PrimitiveState(W[1], W[2], W[3])
end

import Base.+
function +(W1::PrimitiveState, W2::PrimitiveState)
    return PrimitiveState(W1.ρ + W2.ρ, W1.u + W2.u, W1.p + W2.p)
end
...
function *(c::Real, W::PrimitiveState)
    return PrimitiveState(c * W.ρ, c * W.u, c * W.p)
end
...
function ConservedState(; ρ::Real, ρu::Real, E::Real)
    ConservedState(ρ, ρu, E)
end # kwargs
function ConservedState(U::AbstractVector{<:Real})
    length(U) == 3 ||
        throw(ArgumentError("state vector U must only contain 3 elements (ρ, ρu, E)"))
    return ConservedState(U[1], U[2], U[3])
end

import Base.+
function +(U1::ConservedState, U2::ConservedState)
    return ConservedState(U1.ρ + U2.ρ, U1.ρu + U2.ρu, U1.E + U2.E)
end
...
function *(c::Real, U::ConservedState)
    return ConservedState(c * U.ρ, c * U.ρu, c * U.E)
end
...
function +(F1::Flux, F2::Flux)
    return Flux(F1.mass + F2.mass, F1.momentum + F2.momentum, F1.energy + F2.energy)
end
function +(F::Flux, U::ConservedState)
    return Flux(F.mass + U.ρ, F.momentum + U.ρu, F.energy + U.E)
end
function +(U::ConservedState, F::Flux)
    F + U
end
...
function *(c::Real, F::Flux)
    return Flux(c * F.mass, c * F.momentum, c * F.energy)
end
