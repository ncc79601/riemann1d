...
Compute the interface maximum wave speed ``S_\\text{max}`` from left and right states using the specified method.  Used by Riemann solvers that require a two-state estimate.

method:
- `Physical`: ``S_\\text{max} = \\text{max}(|u_L|+a_L,\\, |u_R|+a_R)``
"""
function max_wave_speed(
        W_L::PrimitiveState,
        W_R::PrimitiveState,
        eos::PerfectGasEOS,
...
Fill the flux vector `F` (length `N+1`) with numerical fluxes computed from reconstructed face values `W_L_arr` / `W_R_arr`.
"""
...
Note: allocates one `ConservedState` per cell per step. For zero-allocation operation, swap to `StructArray{ConservedState}` and write component arrays directly.
"""
...
Pre-allocate all work arrays needed by one `evolve!` call and return them as a `NamedTuple` with keys `U_new`, `W`, `F`, `W_L`, `W_R`, `boundaries`, `W_padded`.
"""
...
Run the finite-volume scheme governed by `config.solver` from ``t = 0`` to `config.max_time`. Dispatches to the integrator specified in `config.integrator`.

``U`` is an array of `ConservedState` indexed `1:grid.N` for physical cells. It is mutated in-place and contains the final state after the call.
